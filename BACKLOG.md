...
## Pre-registered: live re-entry test (записано 2026-06-10, ДО события)

Слабое место движка — re-entry после медведя (−141.5пп в окне C walk-forward).
Phase 4 залатал это recovery override'ом, но он ни разу не работал вживую.
Критерий успеха фиксируется СЕЙЧАС, пока мы под SMA200 (−50% от ATH), чтобы в
момент восстановления судить по заранее заданному правилу, а не «на глазок»:

**PASS, если:** с первой строки prod_log, где `recovery_override_would_fire =
true`, `exposure_cap` достигает ≥0.95 в течение ≤5 логовых дней И не
сбрасывается ниже 0.95 в течение следующих 14 дней без bear_confirmation.

**FAIL, если:** target не достигнут за 5 дней, либо ложный сброс.

Проверка: `python analyze_prod_log.py`, секция 2 — статус считается
автоматически (сейчас NOT_YET). Не менять критерий после того, как override
сработает.

## Pre-registered: бейзлайн для движка — SMA200-правило, не HODL

Аудит показал: альфа движка = SMA200-override. Значит нулевая модель для 20k
строк кода — правило «выше SMA200 → 100%, ниже → 35%». analyze_prod_log.py
секция 1 считает три кривые (HODL / SMA200 / движок) на окне лога. Первый
живой замер (2026-06-10, окно 21.04–10.06): HODL −21.5%, SMA200 −7.9%, движок
−8.7% — защита от HODL работает, бейзлайн пока НЕ побит (−0.8пп). Вердикт по
этому вопросу — в конце прод-окна, не раньше.
