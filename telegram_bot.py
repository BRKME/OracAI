...
def action_for(target_pos: float, risk_state: str,
    ...
    # v6.2: процент целевой позиции печатается в TL;DR-буллете, поэтому
    # примечания содержат только ПРИЧИНУ, без повтора цифры.
    ...
    if target_pos >= 0.95:
        ...
        if risk_state == "TAIL":
            return ("🟢 ПОКУПАТЬ", (
                "Дно близко, НО активен хвостовой риск — входи ступенями "
                "лестницы, не на всё сразу; держи резерв и SL под локальный "
                "минимум. Согласуй с риск-блоком выше."))
        return ("🟢 ПОКУПАТЬ", (
            "Сильный сигнал дна — набирай ступенями лестницы "
            "(зона/MVRV/SMA200), не единовременно."))
    if target_pos >= 0.85:
        return ("⚪ ДЕРЖАТЬ", "Базовая HODL-позиция, долгосрочный аптренд BTC.")
    # Умеренная/низкая целевая доля: у ДНА это НЕ продажа — продавать на
    # просадке у дна значит фиксировать убыток против лестницы.
    if near_bottom:
        return ("⚪ ДЕРЖАТЬ", (
            "Медвежья цель по доле, но мы у дна — НЕ продаём на просадке. "
            "Держим базу, добор ступенями лестницы по сигналам дна."))
    if target_pos >= 0.55:
        if dd_from_high < -15 and bear_confirmation:
            return ("🟠 ФИКСИРОВАТЬ",
                    f"Просадка {abs(dd_from_high):.0f}% + медвежье подтверждение.")
        return ("🟠 ФИКСИРОВАТЬ", "Сигнал вершины или TAIL риск.")
    if dd_from_high < -25 and bear_confirmation:
        return ("🔴 ПРОДАВАТЬ",
                f"Глубокая просадка {abs(dd_from_high):.0f}% + подтверждение BEAR.")
    return ("🔴 ПРОДАВАТЬ", "Экстремальная вершина.")
...
def format_output(output: dict, lp_policy=None, allocation=None) -> str:
    ...
    # v6.4: RSI-экстремум БОЛЬШЕ НЕ даёт 100% в одиночку.
    #
    # Проверка на data/btc.csv (16 лет, медиана форвардной доходности 180д):
    #   выборка   RSI<28 против базы
    #   с 2014    -33 пп     (сильно хуже, чем просто держать)
    #   с 2018     +1 пп
    #   с 2021    +11 пп
    # Знак МЕНЯЕТСЯ в зависимости от окна: полноисторический результат целиком
    # создан 2014-2017 годами. Устойчивого преимущества нет ни в одну сторону,
    # а n=126 дней с 2021 — это 3-4 перекрывающихся эпизода, не 126 наблюдений.
    #
    # Поэтому: ни оставлять прыжок 90%→100% на одном индикаторе (данных за это
    # нет), ни переворачивать правило (данных за это тоже нет). Уменьшаем вес
    # непроверяемого сигнала: полный набор требует подтверждения циклом, а
    ...
    buying = target_pos >= 0.95
    if buying:
        # v6.3: когда Действие уже говорит «покупать», строка НЕ должна звать
        # подождать. Расхождение возникает, если сигнал пришёл не из
        # bottom_prox, а из RSI-экстремума: action_for ставит target=1.00 при
        # rsi < 28 независимо от bottom_prox. 17.09 это дало ровно такую пару —
        # «перекос к дну, сигнал слабый, ждём» рядом с «сильный сигнал дна,
        # набирай». Называем источник сигнала явно, чтобы расхождение bottom_prox
        # и RSI было видно, а не выглядело противоречием.
        if rsi_for_check < 28 and bottom_prox <= 0.45:
            cycle_pos_line = (f"📍 По циклу: bottom {_b}% / top {_t}% — сам цикл ещё не на дне, "
                              f"сигнал даёт перепроданность (RSI {rsi_for_check:.0f}) "
                              "→ набор ступенями, не разом")
        else:
