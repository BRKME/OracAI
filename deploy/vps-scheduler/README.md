...
| `oracai-lp-monitor.timer` | ежечасно в :30, 07:30–23:30 | `lp_system.yml` |
...
сделать, а утренний отчёт в 07:00 покажет картину.

Время :30 выбрано не случайно — на :00 этот таймер совпал бы с утренним и
вечерним, а они дёргают тот же сервис. `dispatch.sh` отрабатывает меньше
секунды, systemd не успел бы слить задания, ушло бы два диспатча, и пришёл
бы дубль отчёта.
